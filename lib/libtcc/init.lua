...
local m = {}
m.VERSION = "luajit libtcc 1.1 | tcc version 0.9.28rc 2025-01-23_mob@24f4013 (i386 Windows) with __naked & C++11 raw string literal"
m.VERSION_NUM = 2;
...
m.new = function(output)
    local s = ffi.gc(tcc.tcc_new(), function(cdata)
        local OnGarbageCollection = cdata:get_symbol("OnGarbageCollection", "void(*)()");
        if OnGarbageCollection then
            OnGarbageCollection()
        end
        cdata:delete()
    end)
    ...
    s:add_include_path(libtcc_path .. [[\minhook]])
    s:add_library_path(libtcc_path .. [[\minhook]])
    s:add_library("MinHook.x86")
    ffi.C.LoadLibraryA(libtcc_path .. [[\minhook\MinHook.x86.dll]])
    
    s:add_include_path(libtcc_path .. [[\dcimgui]])
    s:add_library_path(libtcc_path .. [[\dcimgui]])
    s:add_library("dcimgui")
    ffi.C.LoadLibraryA(libtcc_path .. [[\dcimgui\dcimgui.dll]])
